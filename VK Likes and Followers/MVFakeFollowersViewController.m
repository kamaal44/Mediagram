//
//  MVFakeFollowersViewController.m
...
#import "MVFakeFollowersViewController.h"
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.profileName.text = [NSString stringWithFormat:@"%@ %@", self.followers[indexPath.row].first_name, self.followers[indexPath.row].last_name];
    cell.status.text = self.followers[indexPath.row].online.intValue > 0 ? @"(online)" : @"(offline)";
    cell.profilePhoto.layer.borderWidth = 1.0;
    cell.profilePhoto.layer.cornerRadius = cell.profilePhoto.bounds.size.height / 2;
    [cell.profilePhoto sd_setImageWithURL:[NSURL URLWithString:self.followers[indexPath.row].photo_400_orig] placeholderImage:[UIImage imageNamed:@"dog"]];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    alertVC.message = @"Вы желаете потратить 10 монет на просмотр профиля в сети?";
...
- (void)handleNotEnoughCoins {
    ...
    alertVC.message = @"Вам следует получить монеты, чтобы переходить к нужному профилю.";
    alertVC.messageFont = [UIFont fontWithName:@"AvenirNext-Regular" size:17.0];
...
- (void)openSafariWebViewWithIndex:(NSInteger)index {
    SFSafariViewController *svc = [[SFSafariViewController alloc] initWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"https://vk.com/id%@", self.followers[index].id]]];
    svc.delegate = self;
...
- (void)initVKData {
    ...
    VKRequest *request = [VKRequest requestWithMethod:@"friends.getRequests" parameters:nil];
    [SVProgressHUD show];
    ...
    [request executeWithResultBlock:^(VKResponse *response) {
        ...
        [userRequest executeWithResultBlock:^(VKResponse *response) {
            self.followers = [[VKUsersArray alloc] initWithArray:response.json];
